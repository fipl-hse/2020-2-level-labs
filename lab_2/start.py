...
def test_till_calculate_plagiarism_score():
    ...
    lcs_lenght = find_lcs_length(origin_tokens,
                                 susp_tokens,
    ...
    matrix = fill_lcs_matrix(origin_tokens, susp_tokens)
    print(f'A matrix:')
    print(*matrix, sep='\n', end='\n\n')
    
    longest_lcs = find_lcs(origin_tokens, susp_tokens, matrix)
    print(f'The longest common subsequence: {longest_lcs}')

    score = calculate_plagiarism_score(lcs_lenght, susp_tokens)
...
a blue moon will rise'''
    ...
    score = calculate_text_plagiarism_score(origin_tokens,
                                            susp_tokens)
...
def test_find_diff():
    ...
    lcs = find_lcs(origin_tokens, susp_tokens, matrix)

    difference = find_diff_in_sentence(origin_tokens, susp_tokens, lcs)
...
a blue moon will rise'''
    ...
    stat = accumulate_diff_stats(origin_tokens, susp_tokens)
    ...
    report = create_diff_report(origin_tokens,
                                susp_tokens,
                                stat)
    print(f'A report:\n\n{report}\n')
