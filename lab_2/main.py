...
def create_zero_matrix(rows: int, columns: int) -> list:
    ...
    row_columns_int = isinstance(rows, int) and isinstance(columns, int)
    row_columns_bool = isinstance(rows, bool) or isinstance(columns,bool)
    row_colomns_none = rows is not None or columns is not None
    
    if row_columns_int and row_colomns_none and not row_columns_bool and (rows > 0 and columns > 0):
        for i in range(rows):
            zeros = [0] * columns
...
def fill_lcs_matrix(first_sentence_tokens: tuple, second_sentence_tokens: tuple) -> list:
    ...
    m_lcs = create_zero_matrix(len(first_sentence_tokens), len(second_sentence_tokens))
    for ind_1, el_1 in enumerate(first_sentence_tokens):
        for ind_2, el_2 in enumerate(second_sentence_tokens):
            if el_1 == el_2:
                m_lcs[ind_1][ind_2] = m_lcs[ind_1 - 1][ind_2 - 1] + 1
            else:
                m_lcs[ind_1][ind_2] = max(m_lcs[ind_1 - 1][ind_2], m_lcs[ind_1][ind_2 - 1])
    return m_lcs
...
def find_lcs_length(first_sentence_tokens: tuple, second_sentence_tokens: tuple, plagiarism_threshold: float) -> int:
    ...
    m_lcs = fill_lcs_matrix(first_sentence_tokens, second_sentence_tokens)
    if not m_lcs:
        return 0
    len_lcs = m_lcs[-1][-1]
