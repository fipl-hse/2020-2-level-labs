...
def find_diff_in_sentence(original_sentence_tokens: tuple, suspicious_sentence_tokens: tuple, lcs: tuple) -> tuple:
    ...
    if not isinstance(original_sentence_tokens, tuple) or not isinstance(suspicious_sentence_tokens, tuple) or \
            ...
            None in original_sentence_tokens or None in suspicious_sentence_tokens or\
            None in lcs:
        return ()
...
def accumulate_diff_stats(original_text_tokens: tuple, suspicious_text_tokens: tuple, plagiarism_threshold=0.3) -> dict:
    ...
    while len(suspicious_text_tokens) > length:   #equal length
        original_text_tokens += ('',)

    states = {'text_plagiarism': 0,
    ...
    for i in range(length):
        ...
        states['difference_indexes'][i]= find_diff_in_sentence(original_text_tokens[i], suspicious_text_tokens[i],lcs)
    return states
