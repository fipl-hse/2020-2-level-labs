...
def tokenize(text: str) -> list:
    ...
    for i in range(len(string)):  # проверка строки на корректность
        if 48 <= ord(string[i]) <= 57 or (128 <= ord(string[i]) <= 175 or 224 <= ord(string[i]) <= 243):
            k += 1
            break
    ...
    if k != 0:  # если некорректные токены
        ...
        string = string.lower()  # понизили регистр букв (сделали все строчными)
        signs = ",;:\"\'.!?—()-><|"

        for i in range(len(string)):  # цикл по длине строки
            for j in range(len(signs)):  # цикл по длине строки с количеством знаков
                if string[i] == signs[j]:  # если в введенной строке нашли знак препинания
                    string = string[:i] + ' ' + string[
                                                i + 1:]  # берем строку до знака препинания, прибавляем пробел и строку после знака
        return string.split()
...
def remove_stop_words(tokens: list, stop_words: list) -> list:
    ...
    if k != 0:  # если некорректные токены
        ...
        tokens = [token for token in string if token not in stop_words]
        return tokens
...
def get_concordance(tokens: list, word: str, left_context_size: int, right_context_size: int) -> list:
    ...
    if k != 0 or m != 0 or (left_context_size == 0 and right_context_size == 0) or (i - left_context_size < 0 and i + 1 + right_context_size > len(string) - 1):  # если некорректные токены
        ...
        for i in range(len(string)):
            if word == string[i]:
                if i - left_context_size > 0 and i + 1 + right_context_size < len(string) - 1:
                    concordance.append(string[i - left_context_size:i + 1 + right_context_size])
                elif i - left_context_size < 0:
                    concordance.append(string[i:i + 1 + right_context_size])
                elif i + 1 + right_context_size > len(string) - 1:
                    concordance.append(string[i - left_context_size:i + 1])
    return concordance
...
def get_adjacent_words(tokens: list, word: str, left_n: int, right_n: int) -> list:
    ...
    for i in range(len(string)):  # проверка строки на корректность
        if 48 <= ord(' '.join(string)[i]) <= 57 or (128 <= ord(' '.join(string)[i]) <= 175 or 224 <= ord(' '.join(string)[i]) <= 243):
            ...
            break
    m = 0
    for i in range(len(word1)):  # проверка слова на корректность
        if 48 <= ord(word1[i]) <= 57 or (128 <= ord(word1[i]) <= 175 or 224 <= ord(word1[i]) <= 243):
            m += 1
            break

    adjacent_words = []
    if k != 0 or m != 0 or (left_n == 0 and right_n == 0) or (i - left_n < 0 and i + 1 + right_n > len(string) - 1):  # если некорректные токены
        return []
    else:
        for i in range(len(string)):
            if word1 == string[i]:
                if i - left_n > 0 and i + 1 + right_n < len(string) - 1:  # если не выходим за границы
                    adjacent_words.append([string[i - left_n], string[i + right_n]])
                elif i - left_context_size < 0:
                    adjacent_words.append(string[i + right_n])
                elif i + 1 + right_context_size > len(string) - 1:
                    adjacent_words.append(string[i - left_n])
